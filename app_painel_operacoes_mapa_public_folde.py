...
def safe_numeric_series(df: pd.DataFrame, column: str, default: float = 0.0) -> pd.Series:
    """Retorna sempre uma Series numérica, mesmo para DataFrames vazios ou sem a coluna."""
...
def safe_text_series(df: pd.DataFrame, column: str, default: str = "") -> pd.Series:
    """Retorna sempre uma Series textual, mesmo para DataFrames vazios ou sem a coluna."""
    if isinstance(df, pd.DataFrame):
        if column in df.columns:
...
def render_filter_block(
    ...
    key_prefix: str,
    note: str,
    excluded_status_codes_by_default=None,
) -> pd.DataFrame:
    st.markdown(
        f"""
    ...
    with col2:
        ...
        excluded_status_codes = set(excluded_status_codes_by_default or [])
        ...
        status_widget_kwargs = {}
        if status_key in st.session_state:
            current_status_selection = st.session_state.get(status_key, [])
            if not isinstance(current_status_selection, (list, tuple)):
                current_status_selection = []
            sanitized_status_selection = [
                status for status in current_status_selection if status in status_options
            ]
            if list(current_status_selection) != sanitized_status_selection:
                st.session_state[status_key] = sanitized_status_selection
        else:
            status_widget_kwargs["default"] = default_status_options
...
try:
    ...
    st.info(
        "A aplicação permaneceu ativa. Tente selecionar novamente a sessão ou outro arquivo-base."
    )
